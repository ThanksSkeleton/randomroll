...
import {
  ...
  SECTOR_V1_GRID,
  WORLD_ATTRIBUTE_IDS,
} from "../generators/swn_sector/sector_v1";

const ARTIFACT_PATH = resolve("/tmp/randomroll-swn-sector-v1.json");
const ARTIFACT_SEED = "swn-sector-v1-artifact";

describe("SWN sector V1", () => {
  it("generates and writes a valid deterministic sector artifact", () => {
    const sector = generateSectorV1(ARTIFACT_SEED);
    const repeatedSector = generateSectorV1(ARTIFACT_SEED);

    mkdirSync(dirname(ARTIFACT_PATH), { recursive: true });
    writeFileSync(ARTIFACT_PATH, `${JSON.stringify(sector, null, 2)}\n`);

    expect(repeatedSector).toEqual(sector);
    expect(sector.version).toBe("v1");
    ...
    for (const system of sector.systems) {
      ...
      for (const world of system.worlds) {
        ...
        expect(Object.keys(world.attributes)).toEqual([...WORLD_ATTRIBUTE_IDS]);
        expect(Object.values(world.attributes).every(attribute =>
          attribute.roll >= 2 && attribute.roll <= 12 && attribute.result !== "",
        )).toBe(true);
      }
    }
