...
function flattenPowers(powers: FivePowers): string[] {
  ...
  return powers.flatMap((power) => fields.map((field) => power[field]));
}
...
export function build_super(seed: string, num_characters: number): FivePowers[] {
  ...
  for (let i = 0; i < num_characters; i++) {
    ...
    to_return.push([
      first_power,
      second_power,
      three_secondary_powers[0],
      three_secondary_powers[1],
      three_secondary_powers[2],
    ]);
  }
  return to_return;
}
...
function not_duplicate(p: PowersRow, other_powers: PowersRow[]): boolean {
  return other_powers.every(
    (op) =>
      op.Name != p.Name &&
      op.Name != p.variantOf &&
      op.variantOf != p.Name &&
      (op.variantOf != p.variantOf || p.variantOf == ''),
  );
}
...
function first(rng: seedrandom.PRNG): PowersRow {
  ...
  return random_multi(
    rng,
    powers_table.filter((r) => r.Kind == PRIMARY),
  );
}
...
function second(rng: seedrandom.PRNG, first_power: PowersRow): PowersRow {
  ...
  if (Colorless.includes(first_power.Tag)) {
    ...
    color_restricted = non_dupes.filter(
      (r) => Colorless.includes(r.Tag) || r.Tag == first_power.Tag,
    );
...
function three_secondary(
  rng: seedrandom.PRNG,
  first_power: PowersRow,
  second_power: PowersRow,
): [PowersRow, PowersRow, PowersRow] {
