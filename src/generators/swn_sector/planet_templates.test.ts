...
test("every planet template expands to a complete canonical uninhabited planet", () => {
  const templates = ["Mercurian", "Europan / Plutonic", "Lunar", "Ioan", "Titanian", "Martian", "Venusian", "Jovian", "Neptunian"] as const;
  for (const template of templates) {
    const planet = generateTemplatePlanet({ seed: "templates", entityPath: template, starType: "G-type", template, orbit: { AU: 0, AngleDegrees: 0, ParentObjectId: null } });
...
test("equal-temperature direct objects receive distinct in-band AUs", () => {
  const first = generateTemplatePlanet({ seed: "aus", entityPath: "first", starType: "G-type", template: "Ioan", orbit: { AU: 0, AngleDegrees: 0, ParentObjectId: null } });
  const second = generateTemplatePlanet({ seed: "aus", entityPath: "second", starType: "G-type", template: "Ioan", orbit: { AU: 0, AngleDegrees: 1, ParentObjectId: null } });
  ...
  expect(placed.every(object => object.Orbit.AU > minimum && object.Orbit.AU < maximum)).toBe(true);
});

test("system object construction gives moons their parent temperature and AU", () => {
  for (let index = 0; index < 30; index += 1) {
    const system = generateSystem({ seed: `system-${index}`, entityPath: "system:01", hexLocation: { Column: 1, Row: 1 }, starType: "G-type", starHabitability: 3 });
...
test("POIs are assigned only to compatible hosts with bounded capacity", () => {
  for (let index = 0; index < 20; index += 1) {
    const system = populatePointsOfInterest(`pois-${index}`, "system:01", generateSystem({ seed: `pois-${index}`, entityPath: "system:01", hexLocation: { Column: 1, Row: 1 }, starType: "G-type", starHabitability: 3 }));
    ...
    for (const object of system.Objects) expect(system.PointsOfInterest.filter(poi => poi.ParentObjectId === object.Id).length).toBeLessThanOrEqual(3);
  }
});
