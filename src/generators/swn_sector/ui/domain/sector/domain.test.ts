...
describe('sector domain', () => {
  it('updates visibility immutably and reports unknown objects explicitly', () => {
    ...
    expect(updateObjectVisibility(sector, 'missing', VisibilityLevel.BASIC_SCAN)).toEqual({
      ok: false,
  ...
  it('relocates the ship only to a system or world and preserves its source sector', () => {
    ...
    expect(relocatePlayerShip(sector, sector.PlayerShip.Id)).toEqual({
      ok: false,
  ...
  it('cascades deletion without mutating its input and returns a failure for prohibited deletion', () => {
    ...
    expect(deleteSectorObject(sector, sector.PlayerShip.Id)).toEqual({
      ok: false,
      reason: 'deletion-prohibited',
    });
    expect(validateSector(result.value)).toEqual([]);
  });
  ...
  it('applies edits and resolves route travel through focused domain functions', () => {
    ...
    const route = sector.Routes.find(
      (candidate) => candidate.SystemId1 === system.Id || candidate.SystemId2 === system.Id,
    )!;
    const edited = applySectorEdits(sector, {
      sectorName: 'Edited Sector',
      details: { [system.Id]: { NiceName: 'Edited System' } },
    });
