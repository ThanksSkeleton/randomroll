...
import { angleDegrees, chooseWeighted, deterministicId, randomFor, rollDie, shuffled } from "./generation_random";

test("namespaced streams and IDs are deterministic", () => {
  expect([...Array(5)].map(() => rollDie(randomFor("seed", "system:01"), 100))).toEqual([...Array(5)].map(() => rollDie(randomFor("seed", "system:01"), 100)));
  expect(deterministicId("seed", "system:01")).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
