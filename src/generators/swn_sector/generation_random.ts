...
export function chooseWeighted<T extends { Weight: number }>(random: RandomSource, values: readonly T[], context = "weighted values"): T {
  const totalWeight = values.reduce((total, value) => total + value.Weight, 0);
  if (!Number.isFinite(totalWeight) || totalWeight <= 0) throw new Error(`Cannot choose from empty or zero-weight ${context}`);
...
export function deterministicId(seed: string, entityPath: string): string {
  ...
  const words = [0x811c9dc5, 0x01000193, 0x9e3779b9, 0x85ebca6b].map(offset => fnv1a(input, offset));
