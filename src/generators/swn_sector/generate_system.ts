...
export function generateSystem(options: GenerateSystemOptions): StarSystem {
  ...
  return {
    ...
    Intelligence: {
      InfoboxSummary: `${options.starType} system.`,
      BasicScan: `${placed.length} orbiting objects.`,
      CulturePartial: '',
      CultureFull: '',
      GM: '',
    },
    ...
    Star: {
      ...
      VisibilityLevel: 'NONE',
      Intelligence: {
        InfoboxSummary: options.starType,
        BasicScan: `Habitability ${options.starHabitability}.`,
        CulturePartial: '',
        CultureFull: '',
        GM: '',
      },
...
function makePoi(
  ...
  return {
    ...
    Intelligence: {
      InfoboxSummary: type,
      BasicScan: `Located at ${parentObjectId}.`,
      CulturePartial: '',
      CultureFull: '',
      GM: '',
    },
  ...
  for (let index = 0; index < target; index += 1) {
    ...
    if (selected.type === 'Deep-space station') {
      ...
      const station: OtherCelestialObject = {
        ...
        VisibilityLevel: 'NONE',
        Intelligence: {
          InfoboxSummary: 'Independent deep-space station.',
