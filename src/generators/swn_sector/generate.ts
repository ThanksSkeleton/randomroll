...
export function generate(seed: string): Sector {
  ...
  const cells = shuffled(randomFor(seed, "sector:grid"), Array.from({ length: 80 }, (_, index) => ({ Column: index % 10 + 1, Row: Math.floor(index / 10) + 1 })));
  const Systems = cells.slice(0, count).map((hexLocation, index) => {
    ...
    return populatePointsOfInterest(seed, path, generateSystem({ seed, entityPath: path, hexLocation, starType: star.Value as StarType, starHabitability: star.Hab ?? 0 }));
  });
  const { Routes, RoutePortals } = generateRoutes(seed, Systems);
  const shipName = "Player ship";
  return { SchemaVersion: "merged-v1", OriginalSeed: seed, SectorName: `Sector ${seed}`, Systems, Routes, RoutePortals, PlayerShip: { Id: deterministicId(seed, "player-ship"), ProceduralName: shipName, NiceName: shipName, VisibilityLevel: "NONE", Intelligence: { InfoboxSummary: "Player vessel.", BasicScan: "At the first system star.", CulturePartial: "", CultureFull: "", GM: "" }, CurrentLocationId: Systems[0]!.Star.Id } };
}
