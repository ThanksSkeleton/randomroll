...
import { CANONICAL_WORLD_TAGS, EXTRA_WORLD_ARCHETYPES, POI_TABLE, STAR_TABLE, TEMPERATURE_TABLE, WORLD_TAG_TABLE, assertReviewedTableIntegrity, directOrbitAuBand, directOrbitTemperatures } from "./generation_rules";
...
test("compact remnants retain only usable direct-orbit temperature bands", () => {
  expect(directOrbitAuBand("White dwarf", "Temperate")[0]).toBe(directOrbitAuBand("White dwarf", "Temperate")[1]);
  expect(directOrbitTemperatures("White dwarf")).not.toContain("Temperate");
  expect(directOrbitAuBand("White dwarf", "Volcanic")[1]).toBeGreaterThan(directOrbitAuBand("White dwarf", "Volcanic")[0]);
});
