...
function entity(id: string): SelectableEntity {
  return {
    ...
    Intelligence: { InfoboxSummary: "", BasicScan: "", CulturePartial: "", CultureFull: "", GM: "" },
  };
}
...
function planet(id: string, parentObjectId: string | null, size: Planet["Size"]): Planet {
  return {
    ...
    Size: size,
    BulkComposition: size === "Jupiter" ? "Jovian Gas" : size === "Neptune" ? "Neptunian Gas" : "Silicon",
...
function station(id: string, parentObjectId: string | null): OtherCelestialObject {
  return {
    ...
    Orbit: { AU: parentObjectId === null ? 1.4 : 1, AngleDegrees: 45, ParentObjectId: parentObjectId },
  };
}
...
describe("merged-sector independent stations", () => {
  ...
  it("requires a direct station to host exactly one Deep-space station POI", () => {
    ...
    const input = sector([planet("planet", null, "Earth"), planet("extra", null, "Mars"), stationObject]);
    input.Systems[0]!.PointsOfInterest.push({ ...entity("poi"), ParentObjectId: stationObject.Id, POIType: "Deep-space station" });
    const result = checkAllInvariants(input);

    expect(result.some(violation => violation.RuleId === "B21" || violation.RuleId === "F14")).toBe(false);
  });

  it("uses the generator's POI host table", () => {
    const giant = planet("giant", null, "Jupiter");
    const input = sector([giant, planet("moon", giant.Id, "Luna"), station("station", null)]);
    input.Systems[0]!.PointsOfInterest.push({ ...entity("poi"), ParentObjectId: giant.Id, POIType: "Asteroid base" });
  ...
  it("does not use inhabited status to decide POI host compatibility", () => {
    ...
    input.Systems[0]!.PointsOfInterest.push({ ...entity("poi"), ParentObjectId: terrestrial.Id, POIType: "Remote moon base" });
  ...
  it("enforces other-object temperature bands", () => {
    ...
    expect(checkAllInvariants(sector([planet("planet", null, "Earth"), kuiper])).some(violation => violation.RuleId === "F12")).toBe(true);
  });
  ...
  it("uses other-object temperature in direct-orbit ordering", () => {
    ...
    expect(checkAllInvariants(sector([asteroid, coldPlanet])).some(violation => violation.RuleId === "F12")).toBe(true);
  });

  it("treats temperature-band endpoints as exclusive", () => {
    const volcanic = planet("planet", null, "Earth");
    volcanic.Temperature = "Volcanic";
    volcanic.Orbit.AU = .02; // G-type FromStar boundary

    expect(checkAllInvariants(sector([volcanic, station("station", null)])).some(violation => violation.RuleId === "C2")).toBe(true);
  });
  ...
  it("requires rank-1 population for Tomb Worlds and Abandoned Colonies", () => {
    ...
    expect(checkAllInvariants(sector([world, planet("extra", null, "Mars"), station("station", null)])).some(violation => violation.RuleId === "E9")).toBe(true);
  });
  ...
  it("rejects surface water on a Desert World", () => {
    ...
    expect(checkAllInvariants(sector([world, planet("extra", null, "Mars"), station("station", null)])).some(violation => violation.RuleId === "E2")).toBe(true);
  });
  ...
  it("rejects unknown serialized properties", () => {
    const input = sector([planet("planet", null, "Earth"), planet("extra", null, "Mars"), station("station", null)]);
  ...
  it("safely rejects malformed serialized input", () => {
    expect(checkAllInvariants({ SchemaVersion: "merged-v1" }).some(violation => violation.RuleId === "SCHEMA")).toBe(true);
  });
});
