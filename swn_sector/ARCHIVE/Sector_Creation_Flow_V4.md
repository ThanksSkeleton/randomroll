...
V4 extends the V3 star-and-inhabited-world result with a deterministic hierarchy of system locations and location-bound points of interest. The V4 visualizer directly consumes the `SectorV4` object returned by `generateSector`; it does not use a separate view-model transformation. [sector_v4.schema.json](sector_v4.schema.json) is the machine-readable contract for that structured result.
