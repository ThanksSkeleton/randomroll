...
function pageLines(pdfPage) {
  const result = spawnSync(
    'pdftotext',
    ['-f', String(pdfPage), '-l', String(pdfPage), '-bbox-layout', pdf, '-'],
    { encoding: 'utf8' },
  );
  ...
  return [...html.matchAll(/<line xMin="([\d.]+)" yMin="([\d.]+)"[^>]*>([\s\S]*?)<\/line>/g)].map(
    (match) => ({
      x: Number(match[1]),
      y: Number(match[2]),
      text: decodeHtml(
        [...match[3].matchAll(/<word[^>]*>([\s\S]*?)<\/word>/g)].map((word) => word[1]).join(' '),
      ),
    }),
  );
}
    ...
    for (const isLeft of [true, false]) {
      ...
      for (let cardIndex = 0; cardIndex < titleIndices.length; cardIndex += 1) {
        ...
        const description = joinLines(
          card.slice(0, fieldStarts[0]?.index ?? card.length).map((line) => line.text),
        );
        const prompts = Object.fromEntries(
          fieldStarts.map((field, index) => {
            const endIndex = fieldStarts[index + 1]?.index ?? card.length;
            const first = card[field.index].text.replace(/^[EFCTP](?:\s+|$)/, '');
            return [
              field.marker,
              joinLines([first, ...card.slice(field.index + 1, endIndex).map((line) => line.text)]),
            ];
          }),
        );
...
const worldAttributes = {
  ...
  tables: [
    ...
    ...table,
    pdfPage: table.printedPage + 4,
    rows: table.rows.map(([roll, result]) => ({ roll, result })),
  })),
};
  ...
  v4: {
    ...
    pointsOfInterest: {
      ...
      independentOrbit: {
        ...
        allowedOrbitalPositionCategories: ['TooHot', 'TooCold_1', 'TooCold_3'],
      },
    },
  },
  extraWorlds: {
  ...
  otherPoint: {
    ...
    rows: [
      ...
      [
        3,
        'Remote moon base',
        'Any NonPrimary Terrestrial Planet',
        [
      ...
      point,
      locationType,
      occupants: occupants.map((result, index) => ({
        roll: index < 4 ? `${index * 2 + 1}-${index * 2 + 2}` : '9-10',
        result,
      })),
      situations: situations.map((result, index) => ({
        roll: index < 4 ? `${index * 2 + 1}-${index * 2 + 2}` : '9-10',
        result,
      })),
    })),
...
writeFileSync(
  resolve(root, 'world_tags.json'),
  `${JSON.stringify({ source, dice: 'd100', tags: tagCards() }, null, 2)}\n`,
);
writeFileSync(
  resolve(root, 'world_attributes.json'),
  `${JSON.stringify(worldAttributes, null, 2)}\n`,
);
writeFileSync(
  resolve(root, 'system_points_of_interest.json'),
  `${JSON.stringify(systemPoints, null, 2)}\n`,
);
