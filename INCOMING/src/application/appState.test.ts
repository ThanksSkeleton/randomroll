...
describe('application state transitions', () => {
  it('opens a selected system and changes representation atomically', () => {
    ...
    expect(opened).toMatchObject({ view: 'system', currentSystemId: systemId, selectedId: systemId, systemMode: 'symbolic' });
    expect(appReducer(opened, { type: 'changeRepresentation', mode: 'topdown' })).toMatchObject({ view: 'system', currentSystemId: systemId, selectedId: systemId, systemMode: 'topdown' });
  });

  it('discards an edit draft when navigation changes', () => {
    const state = createAppState(createInitialSectors());
    const editing = appReducer(state, { type: 'beginEditing', draft: { sectorName: 'Draft', details: {} } });
  ...
  it('switching preview from archive returns to the sector map', () => {
    ...
    expect(next).toMatchObject({ preview: 'player', view: 'hex', currentSystemId: null, selectedId: null, locked: true });
  });
});
