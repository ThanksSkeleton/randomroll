...
describe("mock sector data", () => {
  it("creates two valid, data-rich deterministic fixtures", () => {
    ...
    for (const sector of first) {
      ...
      expect(getAllSelectableIds(sector).every((id) => /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id))).toBe(true);
      expect(sector.Systems.every(({ HexLocation: { X, Y } }) => X >= 1 && X <= 5 && Y >= 1 && Y <= 8)).toBe(true);
      expect(new Set(sector.Systems.map((system) => system.HexLocation.Y))).toEqual(new Set([1, 2, 3, 4, 5, 6, 7, 8]));
      expect(sector.Routes.every((route) => {
      ...
      expect(sector.Systems.some((system) => system.Worlds.some((world) => world.MoonOf !== null))).toBe(true);
      expect(sector.Systems.every((system) => {
        const planetCount = system.Worlds.filter((world) => world.MoonOf === null).length;
        return planetCount >= 2 && planetCount <= 5;
      })).toBe(true);
      expect(new Set(sector.DetailsAndVisibility.map((detail) => detail.VisibilityLevel))).toEqual(new Set(Object.values(VisibilityLevel)));
  ...
  it("supports lookup, visibility updates, and safe deletion", () => {
    ...
    expect(sector.DetailsAndVisibility.find((detail) => detail.Id === world.Id)?.VisibilityLevel).toBe(VisibilityLevel.CULTURE_FULL);
  ...
  it("cascades world deletion to moons, child POIs, and matching details", () => {
    ...
    expect(sector.DetailsAndVisibility.some((detail) => [world.Id, moon.Id, poi.Id].includes(detail.Id))).toBe(false);
    expect(validateSector(sector)).toEqual([]);
  });

  it("prohibits deleting the active system, stars, and the final system", () => {
    const [sector] = createInitialSectors();
    const activeSystem = sector.Systems.find((system) => system.Id === sector.PlayerShip.CurrentSystemId)!;
  ...
  it("reports invalid IDs, references, and visibility values", () => {
    ...
    expect(validateSector(sector)).toEqual(expect.arrayContaining([
