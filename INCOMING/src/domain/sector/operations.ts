...
export type SectorOperationResult<T> = { ok: true; value: T } | { ok: false; reason: SectorOperationFailure };
...
export function updateObjectVisibility(sector: Sector, id: Guid, level: VisibilityLevel): SectorOperationResult<Sector> {
...
export function relocatePlayerShip(sector: Sector, targetId: Guid): SectorOperationResult<Sector> {
  ...
  if (!target || !target.containingSystem || !["System", "World"].includes(target.kind)) return failure("move-prohibited");
  const next = copySector(sector);
...
export function deleteSectorObject(sector: Sector, id: Guid): SectorOperationResult<Sector> {
  ...
  if (!found || found.kind === "PlayerShip" || found.kind === "Star") return failure("deletion-prohibited");
  const next = copySector(sector);
  const removedIds = new Set<Guid>([id]);

  if (found.kind === "System") {
    if (next.Systems.length <= 1 || next.PlayerShip.CurrentSystemId === id) return failure("deletion-prohibited");
    ...
    const worldAndMoons = new Set([id, ...system.Worlds.filter((candidate) => candidate.MoonOf === id).map((candidate) => candidate.Id)]);
    if (worldAndMoons.has(next.PlayerShip.CurrentLocationId)) return failure("deletion-prohibited");
    for (const world of system.Worlds) if (world.Id === id || world.MoonOf === id) removedIds.add(world.Id);
  ...
  next.DetailsAndVisibility = next.DetailsAndVisibility.filter((detail) => !removedIds.has(detail.Id));
  return success(next);
}

export type EditableDetailField = "NiceName" | "Details_Basic_Scan" | "Details_Culture_Partial" | "Details_Culture_Full" | "Details_GM";
export interface SectorEdits { sectorName: string; details: Record<Guid, Partial<Record<EditableDetailField, string>>>; }
...
export function cloneSectorWithFreshIds(sector: Sector, seed: string, index: number, createId: () => Guid): Sector {
  ...
  for (const system of next.Systems) {
    ...
    for (const world of system.Worlds) { const moonOf = world.MoonOf; world.Id = remap(world.Id); world.MoonOf = moonOf === null ? null : remap(moonOf); }
    for (const poi of system.POIs) { poi.Id = remap(poi.Id); poi.ParentObjectId = remap(poi.ParentObjectId); }
  }
  for (const route of next.Routes) { route.Id = remap(route.Id); route.SystemId1 = remap(route.SystemId1); route.SystemId2 = remap(route.SystemId2); }
